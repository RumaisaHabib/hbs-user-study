...
usi_app.upsell_pids_list_monthly = {
	...
	"33987": 33994
		...
		usi_app.scrape_product_page = function () {
			try {
				...
				if (window.utag_data) {
					...
					// extra
					var extra = {};
					if (plc && utag_data.product_id && utag_data.product_id.length && utag_data.product_id.length >= 3) {
			...
			try {
				if (usi_app.products && usi_app.products.length > 0) {
					...
					usi_app.products.forEach(function (prod) {
						if (prod['plc'] && usi_app.locale == "ko_KR") delete prod['plc'];
		...
		usi_app.check_all_for_suppression = function(prod_array){
			//PLCs: "ACDLTM", "ACDLT", "ACDIST", "RVTLTS", "3DSMAX", "MAYALT", "MAYA", "RVTLT", "AECCOL", "PDCOLL", "MECOLL", "RVT", "CIV3D"
			//end dates NOT INCLUSIVE
			if (!prod_array) return false;
			var is_suppressed = function (dates, locales, plcs,prod) {
				var date = usi_date.is_between(dates[0], dates[1]);
				var locale = locales != null ? locales.indexOf(usi_app.locale) != -1 : true;
				var plc = plcs != null ? plcs.indexOf(prod.plc) != -1 : true;
				return date && locale && plc;
			}
			var suppressed_found = false;
			prod_array.forEach(function(v){
				if(!usi_app.is_forced &&
					(v.name == "Flex"
					//|| is_suppressed(['2022-09-12T08:00:00', '2022-09-16T07:59:00'], ["ja-JP"], ["ACD", "ACDIST", "ACDLT", "RVTLTS"],v)
					)){
		...
		usi_app.get_upsell_info_dynamic = function (prod, cb) {
			try {
				...
				if (prod['plc'] == "ACD" && usi_app.locale == "ko_KR") prod['plc'] = "ACDIST";
				if (prod.product_id && prod.plc && prod.type && prod.qty && typeof cb === "function") {
					...
					if ((["en-US", "en-CA", "fr-CA"].indexOf(usi_app.locale) != -1 && (prod.plc === 'ACDIST' || prod.plc === 'ACDLT')) ||
						...
						(["en-MY", "en-SG", "ko-KR", "ko_KR"].indexOf(usi_app.locale) != -1 && (prod.plc === 'ACDIST' || prod.plc === 'ACDLT'))) {
						...
						//fix korean character for year
						if(usi_app.locale == 'ko_KR' || usi_app.locale == 'ko-KR') bundle_key = bundle_key.substring(0,bundle_key.length - 1) + 'year';
						...
						if (typeof usi_app.bundle_offers[bundle_key] !== "undefined" && usi_app.bundle_offers[bundle_key].qty.indexOf(prod.qty) !== -1) {
							if(usi_app.apac.indexOf(usi_app.locale) != -1 || usi_app.anz.indexOf(usi_app.locale) != -1) {
		...
		usi_app.get_json_extra = function (prod, cb) {
			try {
				if (prod.plc && typeof cb === "function" && usi_app.locale && usi_app.upsell_plcs.indexOf(prod.plc) !== -1) {
					...
					if (usi_app.locale === 'ko_KR') locale = 'ko-KR';
					if (usi_app.locale === 'en_IN') locale = 'en-IN';
					if (usi_app.locale === 'pt_BR') locale = 'pt-BR';
					if (usi_app.locale === 'es_AR') locale = 'es-AR';
					if (usi_app.locale === 'en_SE') locale = 'en-AE';
			...
			try {
				...
				if (typeof (usi_app.options) == "undefined" && usi_commons.device == "desktop" && usi_cookies.get("usi_upsell_displayed") == null) {
					...
					// APAC (no overlay)
						...
						usi_app.usi_load_compat = true;
						if (usi_app.locale === 'ko_KR' && usi_cookies.value_exists('usi_suppress_kr')) {
							usi_commons.log("[USI] Korea Suppress Found");
							return;
						} else if (options.bundle_key) {
							if (usi_cookies.value_exists("usi_aff_suppress")) return;
							usi_commons.log("[USI] bundle identified. Loading 43739");
		...
		usi_app.add_loading_graphic = function (message) {
			try {
				...
				var txt = '<p id="usi_cart_rebuilder_message" style="color: white; font-size: 1.2em; text-align: center; margin-bottom: 20px;">' + message + '</p>';
				var gif = '<!--suppress HtmlUnknownTarget --><img src="' + usi_commons.cdn + '/chatskins/3614/loading.svg" alt="loading gif" style="display: block; margin: 0 auto; height: 30px;">';
				var usi_loading_container = document.createElement('div');
				var wrapper_dynamic_styles = usi_commons.is_mobile ? 'top: 10%; left: 5%; width: 90%; padding: 30px 50px 25px 50px;' : 'top: 280px; left: 30%; width: 40%; padding: 30px 50px 50px 50px;';
				usi_loading_container.innerHTML =
					[
						'<div id="usi_loading_graphic" style="position: fixed; ' + wrapper_dynamic_styles + ' background: rgba(0,0,0,1); border-radius: 5px; z-index: 100000; -webkit-box-shadow: 0 0 81px -9px rgba(0,0,0,0.75); -moz-box-shadow: 0 0 81px -9px rgba(0,0,0,0.75); box-shadow: 0 0 81px -9px rgba(0,0,0,0.75);">',
						txt,
						gif,
						'</div>'
					].join('');
		...
		usi_app.process_cloud_upsells = function () {
			try {
				if (usi_app.usi_cloud_upsell_options) {
					...
					if (usi_app.locale === "en_IN" || usi_app.locale === "en_SE" || usi_app.locale == "en-SG") {
					...
					// Unable to add 1yr to 3yr for en_SE
					if (usi_app.locale === 'en_SE' && usi_app.usi_cloud_upsell_options.term === "3-YEAR") {
						return;
					}
			...
			try {
				if (usi_app.usi_cloud_upsell_options.hash && usi_app.usi_cloud_upsell_options.siteID) {
					...
					if (usi_app.usi_load_compat) {
						...
						if (usi_app.locale === "ko_KR" && document.querySelector('.prod-img img.dr_productThumbnail')) {
			...
			try {
				...
				if (visitor_id && usi_app.aff_links[usi_app.locale] && hash && site && key && term) {
					...
					if (usi_app.locale === 'en-AU') {
						...
						add_url = 'https://checkout.autodesk.com/' + usi_app.locale + '?priceIds=' + priceIds + '&plc=' + plc + '&visitorId=' + visitor_id;
					}
					// -----------------------
					// -------- ja-JP --------
					// -----------------------
						...
						add_url = 'https://checkout.autodesk.com/' + usi_app.locale + '?priceIds=' + priceIds + '&plc=' + plc + '&quantity=1&term=' + term + '&visitorId=' + visitor_id;
					}
					// -----------------------
					// -------- en_IN --------
					// -----------------------
					...
					// -----------------------
						...
						// https://store.autodesk.com/store/adsk/en_SE/buy/productID.5519802300/quantity.1/Currency.USD
						// https://store.autodesk.com/store/adsk/en_SE/buy/productID.5519802400/quantity.1/Currency.USD
					}
						...
						if (i === usi_app.products.length - 1) {
							...
							if (!plc && !prod.plc) {
								...
								usi_app.get_upsell_info_dynamic(prod, function (upsell) {
									...
									// no idea what next line for, but breaks everything
									// looks like it was necessary for the Korean swap? I also have no idea what it's for though. - CB
			...
			try {
				//if (usi_app.upsell_inpage.hash && usi_app.upsell_inpage.siteID) {
					...
					if (typeof callback == "function") {
						...
						window.location = aff + encodeURIComponent(usi_pid_link);
					}
				//}
			} catch (err) {
				usi_commons.report_error(err);
		...
		usi_app.upsell_text_setter = function () {
			...
			try {
				...
				if (["en-NZ", "en-AU"].indexOf(usi_app.locale) != -1) {
				...
				} else if (usi_app.locale == "pt-BR" || usi_app.locale == "pt_BR") {
					...
					options.cta = "P\u00E4ivit\u00E4 nyt";
				} else if (usi_app.locale == "ko-KR" || usi_app.locale == "ko_KR") {
					options.text = "3\uB144 \uC11C\uBE0C\uC2A4\uD06C\uB9BD\uC158\uC73C\uB85C \uBCC0\uACBD\uD558\uC2DC\uACE0, 3\uB144\uAC04 \uD604\uC7AC \uAC00\uACA9\uC73C\uB85C \uACE0\uC815\uD558\uC138\uC694.";
					options.cta = "\uC9C0\uAE08 \uBCC0\uACBD\uD558\uAE30";
				} else if (usi_app.locale == "zh_CN") {
					options.text = "\u8BA2\u8D2D3\u5E74\u671F\u56FA\u5B9A\u671F\u9650\u4F7F\u7528\u8BB8\u53EF\uFF0C\u6301\u7EED\u4EAB\u53D7\u6298\u6263\u529B\u5EA6*\u3002";
					options.cta = "\u70B9\u51FB\u66F4\u65B0\u8D2D\u7269\u8F66\uFF01";
				}
			} catch (err) {
				usi_commons.report_error(err);
		...
		usi_app.has_monthly_keyword = function (type) {
			...
			try {
				return type.indexOf('month') !== -1 ||             // en-US, en-CA
					...
					type.indexOf('mensual') !== -1 ||                  // es_AR
					type.indexOf('mensal') !== -1 ||               // pt_BR
		...
		usi_app.perform_actions_for_dr_payment_page = function (skip_survey) {
			try {
				if (usi_app.suppressf360) return;
				usi_commons.log('[ perform_actions_for_dr_payment_page ] Loading solutions on dr payments page, skip_survey:', skip_survey);
				// Clean up previous solutions
				if (typeof usi_js !== 'undefined' && typeof usi_js.cleanup === 'function') {
					usi_js.cleanup();
				}
				...
				key += '_rebrand';
				if(["zh_CN"].indexOf(usi_app.locale) != -1){
					//Per client request, if we are not in business hours, we will not show any TT
					key = "_phone";
					if(usi_app.is_within_china_business_hours()){
				...
				} else if (["pt_BR", "es_AR"].indexOf(usi_app.locale) != -1) {
				...
				} else if (["ko_KR", "en_IN", "en-MY"].indexOf(usi_app.locale) != -1) {
					if (usi_app.locale === 'ko_KR' && usi_cookies.value_exists('usi_suppress_kr')) return;
					if (usi_commons.is_mobile) usi_commons.load_view("VaTh5jw8Yqu4cmDkcwerSec", "30157", usi_app.locale + key);
					else usi_commons.load_view("q6S0CfQIIS8p5t9KLFJWU6q", "30147", usi_app.locale + key);
				} else if (["en_SE"].indexOf(usi_app.locale) != -1) {
		...
		usi_app.guac_delay_load = function (skip_survey) {
			try {
				...
				if(usi_app.check_all_for_suppression(usi_app.products)) return;
				usi_commons.log('[ guac_delay_load ] Loading solutions on guac payments page, skip_survey:', skip_survey);
				// Clean up previous solutions
				if (typeof usi_js !== 'undefined' && typeof usi_js.cleanup === 'function') {
					usi_js.cleanup();
				}
				...
				if (["en-US", "fr-CA", "en-CA", "es-MX"].indexOf(usi_app.locale) != -1) {
					if (usi_commons.is_mobile) usi_commons.load_view("WQQyBnFTKKbuUrne4X5tHxq", "30151", usi_app.locale + key);
					else usi_commons.load_view("yVgCFhqDcscGL3KqUnbfumi", "30141", usi_app.locale + key);
					...
					var control_site_id = '47311';
					var group = usi_app.force_group || (Math.random() < 0.20 ? 0 : 1);
					if (usi_date.is_after('2023-07-12T08:00')) {
						control_site_id = "47649";
						group = usi_app.force_group || (Math.random() < 0.5 ? 0 : 1);
					}
		...
		usi_app.set_rescrape = function (func, first) {
			try {
				...
				var products = func();
				if (!first && ((usi_app.guac_cart && JSON.stringify(products) !== JSON.stringify(usi_app.products)) || (usi_app.dr_cart && products.length !== usi_app.products.length))) {
			...
			try {
				...
				for (i = 0; i < cart_rows.length; i++) {
					...
					if (product.pic.indexOf("http") != -1) {
						...
						if (product.name.indexOf('AutoCAD - ') !== -1) {
							product.plc = "ACD";
							usi_app.plc = product.plc;
						...
						var name_el = cart_rows[i].querySelector('.product-name.dr_prodName .pd-display-name');
						if (name_el && name_el.textContent) {
							// ACD
							if (name_el.textContent.indexOf('AutoCAD including specialized toolsets') !== -1) {
								usi_app.plc = "ACD";
							} else if (name_el.textContent.indexOf('Media & Entertainment Collection') !== -1) {
								usi_app.plc = "MECOLL";
				...
				for (i = 0; i < cart_rows.length; i++) {
					...
					if (cart_rows[i].getElementsByClassName("checkout--product-bar--info-column--name-sub-column--image-container--badge").length > 0) {
						...
						if (product.pic.indexOf("http") != -1) {
							...
							product.qty = "1";
							var qty_el = cart_rows[i].querySelector('input[name="quantity-' + product.product_id + '"]');
		...
		usi_app.check_for_not_review_page = function (cb) {
			try {
				var review_el = document.querySelector('#QuickBuyConfirmOrderPage .cart-header');
				if (review_el && review_el.textContent && review_el.textContent == "Review Order") {
					setTimeout(function () {
						usi_app.check_for_not_review_page(cb);
					}, 1000);
				} else {
					cb();
				}
			...
			try {
				if (usi_app.efulfilment) {
					...
					try {
						...
						else if ((["en-US", "pt_BR", "es-MX", "en-CA", "fr-CA", "es_AR"]).indexOf(usi_app.locale) != -1) return true;
					} catch (err) {
						usi_commons.report_error(err);
					}
					return false;
					...
					if (payment_section != null && (["en-US", "pt_BR", "es-MX", "en-CA", "fr-CA", "es_AR"]).indexOf(usi_app.locale) == -1) {
			...
			show_opt_in: function (info) {
				try {
					...
					if (document.getElementById("dr_emailInfo") != null) {
						...
						if (usi_app.locale == "pt_BR" || usi_app.locale == "en_IN" || usi_app.locale == "es_AR") {
					...
					if (usi_app.dr_cart == false) {
						css += [
							...
							'#usi_optin_contain {width:100%;top:inherit;bottom:0;position:fixed;left:0;}',
						...
						css += [
							'@media screen and (max-width: 768px) {',
							'#usi_optin_contain {display:none;}',
							'}',
		...
		usi_app.next_question = function () {
			//check + submit answer
			if(usi_js.current_question != -1){
				//check answer
				if(!usi_js.questions[usi_js.current_question].rank) {
					var required_fields_not_answered = document.querySelectorAll('.usi_answer:checked').length === 0 && (document.querySelector(".usi_other") == null || document.querySelector(".usi_other").value == "");
					if (required_fields_not_answered) {
						alert(usi_js.answer_error == null ? "Please select an answer." : usi_js.answer_error);
						return;
					}
				}
				else{
					var required_fields_not_answered = Array.from(document.querySelectorAll('.usi_answer.usi_rank')).filter(function(v){return v.value == ''}).length;
					if (required_fields_not_answered > 0) {
						alert(usi_js.rank_error == null ? "Please rank all options." : usi_js.rank_error);
						return;
					}
				}
				//go through every question and post
				//multiple choice send
				if(!usi_js.questions[usi_js.current_question].rank) {
					usi_js.questions[usi_js.current_question].answers.forEach(function (v, i) {
						usi_app.survey_post({
							questionID: usi_app.question_id,
							question: v + ' [' + usi_app.locale + ']',
							answer: document.querySelector('input[name="answer' + i + '"]').checked ? "yes" : "no",
							freetype: "0"
						});
						usi_app.question_id++;
					});
					if(usi_js.questions[usi_js.current_question].other_field || usi_js.questions[usi_js.current_question].open_field){
						usi_app.survey_post({
							questionID: usi_app.question_id,
							question: "Other" + ' [' + usi_app.locale + ']',
							answer: document.querySelector('input[name="answer'+usi_js.questions[usi_js.current_question].answers.length+'"]').value,
							freetype: "1"
						});
						usi_app.question_id++;
					}
				...
				//rank choice send
					...
					usi_js.questions[usi_js.current_question].answers.forEach(function (v, i) {
						usi_app.survey_post({
							questionID: usi_app.question_id,
							question: v + ' [' + usi_app.locale + ']',
							answer: document.querySelector('input[name="answer' + i + '"]').value,
							freetype: "0"
						});
						usi_app.question_id++;
					});
			...
			//print next question or end screen
			if (usi_js.current_question + 1 < usi_js.questions.length) {
				if (document.querySelector(".usi_closebutton") != null) document.querySelector(".usi_closebutton").remove();
				usi_js.current_question++;
				document.querySelector(".usi_question").dataset.questionid = usi_js.current_question;
				if (!usi_js.questions[usi_js.current_question].rank) usi_app.print_multiple_choice();
				else usi_app.print_rank_choice();
				document.querySelector(".usi_submitbutton").innerText = usi_js.continue_text;
			} else {
				usi_app.end_survey();
			}
		};
		...
		usi_app.print_multiple_choice = function () {
			document.querySelector(".usi_question").innerHTML = "";
			document.querySelector(".usi_title2").innerHTML = usi_js.current_question + 1 + ". " + usi_js.questions[usi_js.current_question].question;
			document.querySelector(".usi_title3").innerHTML = usi_js.questions[usi_js.current_question].subhead;
			usi_js.questions[usi_js.current_question].answers.forEach(function (v, i) {
				var question_html = [
					'<label class="usi_container" for="answer', i, '"><input class="usi_answer" type="checkbox" name="answer', i, '" id="answer', i, '" value="' + v + '"><p class="usi_question_text">' + v + "</p></label>"
				].join("");
				document.querySelector(".usi_question").innerHTML += question_html;
			});
			if (usi_js.questions[usi_js.current_question].other_field) {
				var other_html = [
					'<label class="usi_container" for="answer', usi_js.questions[usi_js.current_question].answers.length, '"><span class="usi_other_title">', usi_js.other_text,
					'</span><input class="usi_other" type="input" name="answer', usi_js.questions[usi_js.current_question].answers.length, '" id="answer', usi_js.questions[usi_js.current_question].answers.length, '" value="" placeholder="',
					usi_js.other_placeholder, '"></label>'
				].join("");
				document.querySelector(".usi_question").innerHTML += other_html;
			}
		};

		usi_app.print_rank_choice = function () {
			document.querySelector(".usi_question").innerHTML = "";
			document.querySelector(".usi_title2").innerHTML = usi_js.current_question + ". " + usi_js.questions[usi_js.current_question].question;
			document.querySelector(".usi_title3").innerHTML = usi_js.questions[usi_js.current_question].subhead;
			usi_js.questions[usi_js.current_question].answers.forEach(function (v, i) {
				var question_html = [
					'<div class="usi_rank_row">',
					'<input class="usi_answer usi_rank" type="number" onkeydown="usi_app.rank_blur(\'down\',',i,')" onkeyup="usi_app.rank_blur(\'up\',',i,')" min="1" max="',usi_js.questions[usi_js.current_question].answers.length,'" name="answer', i, '" id="answer', i, '"/>',
					'<label class="usi_container usi_rank" for="answer', i, '"><p class="usi_question_text">' + v + '</p></label>',
					'</div>'
				].join("");
				document.querySelector(".usi_question").innerHTML += question_html;
			});
		...
		usi_app.rank_blur = function(direction,i){
			var el = document.querySelector("#answer" + i);
			if(direction == 'up') {
				if (el.value > usi_js.questions[usi_js.current_question].answers.length) {
					el.value = usi_js.questions[usi_js.current_question].answers.length;
				} else if (el.value < 1 && el.value != '') el.value = 1;
			}
			else{
				el.value = '';
			}
		...
		usi_app.end_survey = function () {
			document.querySelector("#usi_content").innerHTML = usi_js.display_vars.p2_html;
			usi_js.post_close = usi_js.same_page_link;
		};
		...
		usi_app.survey_post = function(data) {
			...
			qs += "&question=" + encodeURIComponent(data.question) + "&answer=" + encodeURIComponent(data.answer);
		...
		usi_app.is_within_china_business_hours = function (){
			...
			date.setMinutes(date.getMinutes() + date.getTimezoneOffset() + 480); // Offset CST
			var hours = date.getHours();
			var minutes = date.getMinutes();
			var day = date.getDay();
			var openingDays = [ 1, 2, 3, 4, 5 ]; // M-F
			return openingDays.includes(day) && ((hours >= 10 && hours <= 16) || (hours == 9 && minutes >= 30) || (hours == 17 && minutes < 30)); //CST M-F 9:30 AM - 5:30 PM
		};
